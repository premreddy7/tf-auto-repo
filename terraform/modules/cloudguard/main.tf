...
resource "google_compute_instance_template" "instance_template" {
  ...
  tags = [
    ...
    local.network_defined_by_routes_settings_condition, var.region
  ]

  metadata_startup_script = templatefile("../modules/cloudguard/startup-script.sh", {
    // script's arguments
...
resource "google_compute_region_instance_group_manager" "instance_group_manager" {
  region = var.region
  name   = "${var.prefix}-${substr(md5(google_compute_instance_template.instance_template.name), 0, 3)}"
  version {
...
resource "google_compute_region_autoscaler" "autoscaler" {
  region = var.region
  name   = "${var.prefix}-autoscaler-${substr(md5(google_compute_instance_template.instance_template.name), 0, 3)}"
